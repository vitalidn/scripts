...
#python3 config_sysctl_args_cli.py -h   

""" python3 sysctl_apply_cli.py \
  -hosts db-[1:10].corp.local \
  -p 'password' \
  -c ./new_sysctl_args.yaml \
  -max 8
"""
...
def keyscan_known_hosts(hosts: List[str]) -> None:
    logging.info(f"[keyscan] Starting SSH key scan for {len(hosts)} hosts…")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Verify & apply sysctl settings on multiple hosts")
    p.add_argument(
        "-hosts", "--hosts-mask",
        required=True,
        help="Hosts mask e.g. prefix-[1:100].domain"
    )
    p.add_argument(
        "-p", "--sudo-password",
        required=True,
        help="Sudo password for remote execution"
    )
    p.add_argument(
        "-c", "--config-file",
        type=Path,
        default=Path.cwd() / "new_sysctl_args.yaml",
        help="Path to YAML config file (default: ./new_sysctl_args.yaml)"
    )
    p.add_argument(
        "-max", "--max-hosts",
        type=int,
...
def main() -> None:
    ...
    try:
        hosts = parse_node(args.hosts_mask)
    except Exception as e:
        print(f"Error parsing hosts mask: {e}")
        return
